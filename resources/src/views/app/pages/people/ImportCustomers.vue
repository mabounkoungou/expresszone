        ...
        <b-col md="7" class="mb-4">
          ...
          <b-card class="mt-3">
            ...
            <ul class="mini-notes mt-2">
              <li><strong>Column names are case-insensitive</strong> and spaces are optional (e.g., "first name" or "firstname" both work).</li>
              <li><strong>name</strong> (or username/customer) is required.</li>
              <li><strong>code</strong> must be an integer and unique.</li>
              ...
              <li><strong>adresse</strong> (or address/addr) is the street address field.</li>
              <li><strong>opening_balance</strong> is optional; if present must be numeric.</li>
              <li><strong>Important:</strong> Use only the column name (e.g., "code"), not descriptions like "code (integer)".</li>
            </ul>
          </b-card>
          ...
          <!-- DEBUG PANEL (when available) -->
          <b-alert v-if="debugInfo" show variant="info" class="mt-3" style="border: 2px solid #0c5460;">
            <div class="d-flex align-items-start">
              <lucide-icon class="mr-2 mt-1" name="info" />
              <div style="width: 100%; font-size: 0.9rem;">
                <div class="font-weight-bold mb-2" style="font-size: 1.1rem;">📊 Import Debug Information:</div>
                <div v-if="debugInfo.total_rows_in_file !== undefined" style="background: #e7f3ff; padding: 12px; border-radius: 6px;">
                  <strong>File Structure Analysis:</strong>
                  <div style="margin-top: 8px; font-family: monospace; line-height: 1.8;">
                    <div>📄 Total rows in file: <strong>{{ debugInfo.total_rows_in_file }}</strong></div>
                    <div>✓ Normalized rows: <strong>{{ debugInfo.normalized_rows }}</strong></div>
                    <div>✓ Prepared rows: <strong>{{ debugInfo.prepared_rows }}</strong></div>
                    <div>✓ Inserted rows: <strong style="color: green;">{{ debugInfo.inserted_rows }}</strong></div>
                  </div>
                  <div style="margin-top: 12px; font-size: 0.85rem; color: #666;">
                    <strong>Interpretation:</strong>
                    <ul style="margin-bottom: 0;">
                      <li v-if="debugInfo.total_rows_in_file <= 1">⚠️ Only {{ debugInfo.total_rows_in_file }} row found - likely headers only, no data rows</li>
                      <li v-if="debugInfo.normalized_rows === 0 && debugInfo.total_rows_in_file > 1">⚠️ Data rows not being extracted - check column names match expected format</li>
                      <li v-if="debugInfo.prepared_rows === 0 && debugInfo.normalized_rows > 0">⚠️ Data rejected during validation - check required fields (name, code)</li>
                      <li v-if="debugInfo.inserted_rows === 0 && debugInfo.prepared_rows > 0">⚠️ Database insertion failed - check for duplicate codes/emails</li>
                      <li v-if="debugInfo.inserted_rows > 0">✅ Success! Customers were imported.</li>
                    </ul>
                  </div>
                </div>
              </div>
  ...
  return out
    ...
    async submit() {
      ...
      try {
        ...
        if (count === 0) {
          // Show debug info prominently when 0 customers imported
          this.debugInfo = data.debug || {
            total_rows_in_file: 'unknown',
            normalized_rows: 'unknown',
            prepared_rows: 'unknown',
            inserted_rows: 0,
          };
          this.toast('No customers were imported. Please check your file format.', 'Warning', 'warning');
          return;
        }
        ...
        this.toast(count + ' customer' + (count !== 1 ? 's' : '') + ' imported successfully.', 'Success', 'success');
        
        // Redirect after a brief delay to ensure success message is visible
        setTimeout(() => {
