  ...
  methods: {
    ...
    async CreatePOS() {
      ...
      try {
        ...
        if (isNetworkError) {
          const msg = this.$t
            ? (this.$t('NetworkErrorTryAgain') || 'Network error, please try again.')
            : 'Network error, please try again.';
          this.makeToast && this.makeToast('danger', msg, this.$t ? this.$t('Failed') : 'Failed');
        } else {
          const responseData = error.response && error.response.data;
          const validationErrors = responseData && responseData.errors;
          const firstValidationError = validationErrors
            ? Object.values(validationErrors).reduce((message, messages) => message || (Array.isArray(messages) ? messages[0] : messages), '')
            : '';
          const msg = firstValidationError || (responseData && responseData.message) || (this.$t ? this.$t('InvalidData') : 'Invalid data');
