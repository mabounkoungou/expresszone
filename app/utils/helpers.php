...
class helpers
{
    ...
    public function filter($model, $columns, $param, $request)
    {
        ...
        foreach ($fields as $field) {
            $model = $model->when($request->filled($field['value']),
                function ($query) use ($request, $field) {
                    $value = $request->get($field['value']);
                    $column = $field['value'];
                    ...
                    if (strpos($column, '.') === false) {
                        try {
                            ...
                            // Fall back to unqualified field names if the model is unavailable.
                        }
                    }
