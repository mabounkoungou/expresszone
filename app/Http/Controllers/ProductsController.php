...
class ProductsController extends BaseController
{
    ...
    public function store(Request $request)
    {
        ...
        try {
            ...
            $categoryPrices = json_decode($request->input('category_prices', '[]'), true) ?: [];
            if ($categoryPrices === []) {
                $categoryPrices = [[
            ...
            foreach ($categoryPrices as $categoryPrice) {
                if (empty($categoryPrice['category_id']) || ! is_numeric($categoryPrice['price'])) {
                    return response()->json(['message' => 'Each category must have a valid amount.'], 422);
                }
            }

            \DB::transaction(function () use ($request, $categoryPrices) {
                ...
                $firstCategory = $categoryPrices[0];
                ...
                if ($request['type'] == 'is_single' || $request['type'] == 'is_combo') {
                    ...
                    $Product->price = $firstCategory['price'];
                    $Product->wholesale_price = ! empty($request['wholesale_price']) ? $request['wholesale_price'] : 0;
                    $Product->min_price = ! empty($request['min_price']) ? $request['min_price'] : 0;
                    $Product->cost = 0;
                ...
                $Product->categories()->sync([$firstCategory['category_id']]);

                foreach (array_slice($categoryPrices, 1) as $categoryPrice) {
                    $categoryProduct = $Product->replicate();
                    $categoryProduct->category_id = $categoryPrice['category_id'];
                    $categoryProduct->price = $categoryPrice['price'];
                    $categoryProduct->code = $this->generateServiceCode($request['name'], $categoryPrice['category_id']);
                    $categoryProduct->save();
                    $categoryProduct->categories()->sync([$categoryPrice['category_id']]);
                }
    ...
    public function update(Request $request, $id)
    {
        ...
        try {
            ...
            \DB::transaction(function () use ($request, $id) {
                ...
                if ($request->has('category_prices')) {
                    $categoryPrices = json_decode($request->input('category_prices', '[]'), true) ?: [];
                    foreach ($categoryPrices as $categoryPrice) {
                        if (empty($categoryPrice['category_id']) || ! is_numeric($categoryPrice['price'])) {
                            continue;
                        }
                        if ((int) $categoryPrice['category_id'] === (int) $Product->category_id) {
                            $Product->price = $categoryPrice['price'];
                            $Product->saveQuietly();
                            continue;
                        }
                        $categoryProduct = Product::where('id', '!=', $Product->id)
                            ->where('category_id', $categoryPrice['category_id'])
                            ->where('name', $Product->name)
                            ->whereNull('deleted_at')
                            ->first();
                        if ($categoryProduct) {
                            $categoryProduct->price = $categoryPrice['price'];
                            $categoryProduct->saveQuietly();
                        } else {
                            $categoryProduct = $Product->replicate();
                            $categoryProduct->category_id = $categoryPrice['category_id'];
                            $categoryProduct->price = $categoryPrice['price'];
                            $categoryProduct->code = $this->generateServiceCode($Product->name, $categoryPrice['category_id']);
                            $categoryProduct->Type_barcode = 'CODE128';
                            $categoryProduct->saveQuietly();
                            $categoryProduct->categories()->sync([$categoryPrice['category_id']]);
                        }
                    }
    ...
    public function Get_Products_Details(Request $request, $id)
    {
        ...
        $item['category_prices'] = Product::where('name', $Product->name)
            ->whereNull('deleted_at')
            ->get(['category_id', 'price'])
            ->map(fn($categoryProduct) => [
     ...
     */
    protected function generateServiceCode(string $name, $categoryId): string
    {
        $category = Category::find($categoryId);
        $parts = [$name, $category ? $category->name : 'CATEGORY'];
        $base = strtoupper(trim(preg_replace('/[^A-Za-z0-9]+/', '-', implode('-', $parts)), '-')) ?: 'SERVICE-CATEGORY';
        $number = 1;

        do {
            $code = $base . '-' . str_pad((string) $number, 4, '0', STR_PAD_LEFT);
            $used = Product::where('code', $code)->whereNull('deleted_at')->exists()
                || ProductVariant::where('code', $code)->whereNull('deleted_at')->exists();
            $number++;
        } while ($used);
