...
class ReportQuestionController extends BaseController
{
    ...
    public function index(Request $request)
    {
        ...
        $this->authorizeForUser($request->user('api'), 'business_insights_view', Client::class);
    ...
    {
        // Authorization - check if user has AI Reports access
        $this->authorizeForUser($request->user('api'), 'business_insights_run', Client::class);
    ...
    public function businessInsights(Request $request)
    {
        ...
        $allowed = $user->is_all_warehouses
            ? Warehouse::whereNull('deleted_at')->pluck('id')->all()
            : UserWarehouse::where('user_id', $user->id)->pluck('warehouse_id')->all();
        if ($request->warehouse_id && ! in_array((int) $request->warehouse_id, array_map('intval', $allowed), true)) {
            abort(403, 'You are not assigned to this branch.');
        }

        return response()->json($this->reportService->businessInsights(
            $request->date_from,
            $request->date_to,
