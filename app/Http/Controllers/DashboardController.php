...
class DashboardController extends Controller
{
    ...
    public function dashboard_data(Request $request)
    {
        ...
        if (empty($request->warehouse_id)) {
            ...
            if (! in_array((int) $warehouse_id, array_map('intval', $array_warehouses_id), true)) {
    ...
    public function SalesChart($warehouse_id, $array_warehouses_id, $from = null, $to = null)
    {
        ...
        $view_records = $user->hasRecordView() || ! $user->is_all_warehouses;
    ...
    public function report_dashboard($request, $warehouse_id, $array_warehouses_id)
    {
        ...
        $data['money_collected'] = $today_sales_paid;
        $data['money_remaining'] = $today_sales_due_amount;
        $data['sales_due'] = $today_sales_due_amount;

        $moneyTakenPermission = Permission::where('name', 'money_taken_view')->first();
        $canViewMoneyTaken = $moneyTakenPermission && $user->hasRole($moneyTakenPermission->roles);
        if ($canViewMoneyTaken) {
            $takenQuery = MoneyTaken::where(function ($query) use ($warehouse_id, $array_warehouses_id, $user) {
                $query->whereIn('warehouse_id', $warehouse_id === 0 ? $array_warehouses_id : [$warehouse_id]);
                if ($user->is_all_warehouses) {
                    $query->orWhereNull('warehouse_id');
                }
            });
            $salesTakenQuery = Sale::whereNull('deleted_at')->whereIn('warehouse_id', $warehouse_id === 0 ? $array_warehouses_id : [$warehouse_id]);
            $data['total_taken'] = (float) $takenQuery->sum('amount');
            $data['taken_remaining'] = max(0, (float) $salesTakenQuery->sum('paid_amount') - $data['total_taken']);
        } else {
            $data['total_taken'] = null;
            $data['taken_remaining'] = null;
        }
