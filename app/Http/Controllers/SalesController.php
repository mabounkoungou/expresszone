...
class SalesController extends BaseController
{
    ...
    public function Print_Invoice_POS(Request $request, $id)
    {
        ...
        $sale = Sale::with('details.product.unitSale', 'details.product.category')
            ->where('deleted_at', '=', null)
            ->findOrFail($id);
        ...
        foreach ($sale['details'] as $detail) {
            ...
            $data['category_name'] = optional($detail['product']->category)->name;
    ...
    public static function normalizeWarehouseReferenceName($warehouseName): string
    {
        $warehouseName = strtoupper(trim(preg_replace('/[^A-Za-z0-9]+/', '-', (string) $warehouseName), '-'));

        return $warehouseName !== '' ? $warehouseName : 'WAREHOUSE';
    }

    public static function resolveSaleReferencePrefix($prefix, $warehouseId = null): string
    {
        ...
        if ($warehouseId) {
            ...
            if ($warehouse && ! empty($warehouse->name)) {
                $warehouseName = self::normalizeWarehouseReferenceName($warehouse->name);
            }
        }

        $prefix = str_ireplace('WAREHOUSENAME', $warehouseName, $prefix);
        $prefix = str_ireplace('WAREHOUSE_NAME', $warehouseName, $prefix);

        $prefix = preg_replace('/[^A-Za-z0-9-]+/', '', $prefix);
        $prefix = trim((string) $prefix, '-');

        return strtoupper($prefix !== '' ? $prefix : 'SL');
    }

    public static function buildNextSaleReference($prefix, $lastReference = null): string
    {
        $cleanPrefix = self::resolveSaleReferencePrefix($prefix);
        $suffixNumber = 1;

        if (is_string($lastReference) && $lastReference !== '' && preg_match('/-(\d+)$/', $lastReference, $matches)) {
            $suffixNumber = (int) $matches[1] + 1;
        }

        return $cleanPrefix . '-' . str_pad((string) $suffixNumber, 4, '0', STR_PAD_LEFT);
    }

    public function getNumberOrder($warehouseId = null)
    {
        if ($warehouseId === null) {
        ...
        $prefix = self::resolveSaleReferencePrefix($setting->sale_prefix ?? 'SL', $warehouseId);
        ...
        return self::buildNextSaleReference($prefix, $last?->Ref);
    }
