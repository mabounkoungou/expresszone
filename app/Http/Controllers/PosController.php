...
class PosController extends BaseController
{
    ...
    public function CreatePOS(Request $request)
    {
        ...
        try {
            $sale = \DB::transaction(function () use ($request, $totalPaid, $saleUuid) {
                ...
                $order->Ref = $this->getPosInvoiceReference($request->warehouse_id);
    ...
    private function getPosInvoiceReference($warehouseId): string
    {
        $warehouse = Warehouse::whereNull('deleted_at')->findOrFail($warehouseId);
        $warehouseName = strtoupper(trim(preg_replace('/[^A-Za-z0-9]+/', '-', $warehouse->name), '-'));
        $warehouseName = $warehouseName !== '' ? $warehouseName : 'WAREHOUSE';
        $prefix = 'EZL-' . $warehouseName . '-';

        $lastReference = Sale::where('Ref', 'like', $prefix . '%')
            ->orderByDesc('id')
            ->value('Ref');
        $lastNumber = 0;
    ...
    public function GetProductsByParametre(request $request)
    {
        ...
        $product_warehouse_query = product_warehouse::where('warehouse_id', $request->warehouse_id)
            ->with('product', 'product.unitSale', 'product.category')
            ->where('deleted_at', '=', null)
        ...
        foreach ($product_warehouse_data as $product_warehouse) {
            ...
            $item['category_name'] = optional($product_warehouse['product']->category)->name;
