...
class ClientController extends BaseController
{
    ...
    public function index(request $request)
    {
        ...
        // Apply warehouse filtering if user doesn't have access to all warehouses
        if (!$user->is_all_warehouses) {
            $userWarehouseIds = $user->assignedWarehouses()->pluck('warehouse_id')->toArray();
            $clients = $clients->where(function ($query) use ($userWarehouseIds) {
                // Show customers assigned to user's warehouses OR customers with null warehouse_id (backward compatibility)
        ...
        foreach ($clients as $client) {
            ...
            $item['warehouse_id'] = $client->warehouse_id;
    ...
    public function store(Request $request)
    {
        ...
        // - If user has all warehouses permission, use provided warehouse_id or null
        // - If user is restricted to specific warehouses, auto-assign to their first warehouse
        $warehouse_id = $request['warehouse_id'] ?? null;
        if (!$user->is_all_warehouses) {
            $userWarehouses = $user->assignedWarehouses()->pluck('warehouse_id')->toArray();
            if (!empty($userWarehouses)) {
                // Auto-assign to user's warehouse (use first if multiple)
                $warehouse_id = $userWarehouses[0];
            }
        }
    ...
    public function import(Request $request)
    {
        ...
        // Initialize debug info that we'll track throughout
        $debugInfo = [
        ...
        // Track total rows
        $debugInfo['total_rows_in_file'] = count($sheet);

        \Log::debug('Excel Import - File loaded', [
            'total_sheets' => count($rows),
            'sheet_rows' => count($sheet),
        ]);
        ...
        // CustomerImport uses WithHeadingRow, so data should be associative
        // But handle both cases to be safe
        $normalized = [];

        // Check if we have associative data (first row has string keys)
        $first = reset($sheet);
        $hasStringKeys = is_array($first) && count(array_filter(array_keys($first), 'is_string')) > 0;

        \Log::debug('Excel Import - Row analysis:', [
            'first_row_is_array' => is_array($first),
            'has_string_keys' => $hasStringKeys,
            'first_row_keys' => is_array($first) ? array_keys($first) : [],
        ]);

        if ($hasStringKeys) {
            // Data is already associative (WithHeadingRow worked correctly)
            foreach ($sheet as $row) {
                if (is_array($row) && !empty(array_filter($row))) {
                    $normalized[] = $this->normalizeAssocRow($row);
                }
            }
        } else {
            // Data is numeric, first row should be headers
            if (empty($sheet) || !is_array($sheet[0])) {
                $debugInfo['total_rows_in_file'] = count($sheet);
                return response()->json([
                    'status' => false,
                    'message' => 'Could not parse the Excel file.',
                    'errors' => ['The file format is not supported or contains no valid data.'],
                    'debug' => $debugInfo,
                ], 422);
            }
            ...
            if (empty(array_filter($headers))) {
                ...
                return response()->json([
                    ...
                    'errors' => ['The first row appears to be empty or contains no valid headers.'],
                    'debug' => $debugInfo,
                ], 422);
            }
            ...
            for ($i = 1; $i < count($sheet); $i++) {
                $dataRow = $sheet[$i];
                if (!is_array($dataRow) || empty(array_filter($dataRow))) continue;

                $assoc = [];
                foreach ($headers as $idx => $key) {
                    $assoc[$key] = $dataRow[$idx] ?? null;
                }
                $normalized[] = $this->normalizeAssocRow($assoc);
            }
        }

        // Update debug info after normalization
        $debugInfo['normalized_rows'] = count($normalized);
        ...
        if (empty($normalized)) {
            \Log::warning('Excel Import - No normalized rows found', [
                'sheet_rows' => count($sheet),
                'hasStringKeys' => isset($hasStringKeys) ? $hasStringKeys : null,
            ]);
            return response()->json([
                'status' => false,
                'message' => 'No data rows found in the file.',
                'errors' => ['The file contains only headers with no customer data. Please add at least one row of customer data below the header row.'],
                'debug' => $debugInfo,
            ], 422);
        }

        \Log::info('Excel Import - Processing normalized rows', ['count' => count($normalized)]);
        ...
        foreach ($normalized as $i => $row) {
            ...
            $warehouse_id_raw = $row['warehouse_id'] ?? null;
            ...
            // warehouse_id: optional, must be valid if provided
            $warehouse_id = null;
            if ($warehouse_id_raw !== null && $warehouse_id_raw !== '') {
                if (is_numeric($warehouse_id_raw) && intval($warehouse_id_raw) == $warehouse_id_raw) {
                    $warehouseIdToCheck = intval($warehouse_id_raw);
                    // Verify warehouse exists
                    if (! \App\Models\Warehouse::where('id', $warehouseIdToCheck)->exists()) {
                        $errors[] = "Row {$line}: warehouse_id '{$warehouseIdToCheck}' does not exist in the system.";
                    } else {
                        $warehouse_id = $warehouseIdToCheck;
                    }
                } else {
                    $errors[] = "Row {$line}: warehouse_id '{$warehouse_id_raw}' is not a valid integer.";
                }
            }
        ...
        // Update debug info after validation
        $debugInfo['prepared_rows'] = count($prepared);

        // Log validation results
        \Log::info('Excel Import - Validation complete', [
        ...
        // If there are validation errors OR no data was successfully prepared, return error
        if (! empty($errors)) {
            \Log::warning('Excel Import - Validation failed', ['errors' => $errors]);
            return response()->json([
                'status' => false,
                'message' => 'Validation failed',
                'errors' => $errors,
        ...
        // Final check: ensure we have at least one row prepared
        if (empty($prepared)) {
            return response()->json([
                'status' => false,
                'message' => 'No valid customer records to import.',
                'errors' => ['All rows in the file failed validation. Please check the file format and data.'],
                'debug' => $debugInfo,
            ], 422);
        }
        ...
        if (empty($insertRows)) {
            return response()->json([
                ...
                'errors' => ['The file appears to have no valid customer records.'],
                'debug' => $debugInfo,
            ], 422);
        }
        ...
        try {
            ...
            \Log::error('Excel Import - Database insert failed', ['error' => $e->getMessage()]);
            return response()->json([
                'status' => false,
        ...
        // Update debug info with inserted count
        $debugInfo['inserted_rows'] = count($insertRows);
