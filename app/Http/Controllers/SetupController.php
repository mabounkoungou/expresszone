...
class SetupController extends Controller
{
    ...
    public function lastStep(Request $request)
    {
        ...
        $log = escapeshellarg(storage_path('logs/setup-install.log'));
        $command = "start /B \"\" {$php} {$artisan} setup:install > {$log} 2>&1";
    ...
    {
        $status = Storage::disk('public')->get('setup-status.json');

        return response()->json(json_decode($status ?: '{"status":"pending"}', true));
    }
